        ...
        function (require, module, exports) {
          ...
          function feature(options, styleHashesArray) {
            return function (_) {
              ...
              if (options.simpleStyle) {
                ...
                if (styleHash) {
                  if (
                    geometry.isPoint(_.geometry) &&
                    hasMarkerStyle(_.properties)
                  ) {
                    ...
                    (geometry.isPolygon(_.geometry) ||
                      geometry.isLine(_.geometry)) &&
                    hasPolygonAndLineStyle(_.properties)
                  ) {
                    if (styleHashesArray.indexOf(styleHash) === -1) {
                      styleDefinition = polygonAndLineStyle(
                        _.properties,
                        styleHash
                      )
                      styleHashesArray.push(styleHash)
          ...
          function root(_, options) {
            ...
            switch (_.type) {
              case 'FeatureCollection':
                ...
                return _.features
                  .map(feature(options, styleHashesArray))
                  .join('')
              case 'Feature':
          ...
          function markerStyle(_, styleHash) {
            return tag(
              'Style',
              { id: styleHash },
              tag('IconStyle', tag('Icon', tag('href', iconUrl(_)))) +
                iconSize(_)
            )
          }
          ...
          function hashStyle(_) {
            ...
            if (_['stroke-opacity'])
              hash =
                hash + 'mo' + _['stroke-opacity'].toString().replace('.', '')
          ...
          function tag(el, attributes, contents) {
            ...
            if (Array.isArray(contents))
              contents =
                ...
                '\n'
            return (
              '<' + el + attr(attributes) + '>' + contents + '</' + el + '>'
            )
          }
        },
        { './xml-escape': 3 }
